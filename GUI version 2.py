...
def frame1():
    ...
    profile_img=tk.PhotoImage(file="profile.png")
    img_label=tk.Label(frame1,image=profile_img)
    img_label.grid(row=0,column=0)

    #placing the objects in frame1
    lblsign_up = Label (frame1, text = "Sign Up")
    lblsign_up.grid(row=6, column=0)

    
    lblUser = Label (frame1,text = "Username")
    lblUser.grid(row=7,column=0)

    entryUser=Entry (frame1)
    entryUser.grid (row=7,column=1)

    lblPass = Label (frame1,text="Password")
    lblPass.grid (row= 8, column=0)
